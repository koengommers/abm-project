...
class CanvasContinuous(VisualizationElement):
    """Canvas class needed to draw coninuous space model in the browser. """
    local_includes = ['js/CanvasContinuousModule.js',
                      'js/ContinuousVisualization.js']
    ...
    def render(self, model):
        """Renders model visualization for each time step. """
        ...
        for agent in model.space._agent_to_index:
            ...
            if portrayal['Shape'] == 'circleWithTrail':
                ...
                fromx = (fromx - model.space.x_min) / \
                    (model.space.x_max - model.space.x_min)
                fromy = (fromy - model.space.y_min) / \
                    (model.space.y_max - model.space.y_min)
